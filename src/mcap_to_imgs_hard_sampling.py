...
with minimum time gap enforcement (Hard Sampling).

This addresses temporal redundancy by ensuring frames are spaced apart in time,
avoiding the inefficiency of training on nearly-identical consecutive frames.

Key difference from Gaussian sampling: After sampling from the Gaussian distribution,
this enforces a minimum time gap between frames, removing temporally redundant samples.
"""
...
def find_matching_pairs(color_msgs: List[Tuple], depth_msgs: List[Tuple], 
    ...
    """
    Find matching color-depth pairs based on timestamp proximity.
    
    Args:
        color_msgs: List of (timestamp, message) tuples for color images
        depth_msgs: List of (timestamp, message) tuples for depth images
        max_time_diff_ns: Maximum time difference in nanoseconds
        
    Returns:
        List of (color_index, depth_index) pairs
    """
    pairs = []
    depth_idx = 0
...
def extract_frames_hard_sampling(mcap_path: str, 
                                 color_topic: str, 
                                 depth_topic: str,
                                 num_frames: int,
                                 target_timestamps: List[int],
                                 ...
                                 seed: int = None):
    """
    Extract color and depth image pairs using Gaussian-weighted sampling with minimum time gap.
    
    This implements "hard sampling" to avoid temporal redundancy:
    1. Sample from Gaussian distribution (focusing on important events)
    2. Enforce minimum time gap to ensure diverse frames
    
    Args:
        mcap_path: Path to the MCAP file
        color_topic: Topic name for color images
        depth_topic: Topic name for depth images
        num_frames: Desired number of frames (actual number may be less after filtering)
        ...
        min_gap_seconds: Minimum time gap between frames in seconds
    ...
    output_dir = output_base_dir / f"{mcap_name}_hard_sampling_{timestamp}"
    ...
    # Oversample by a factor to ensure we get close to num_frames after filtering
    oversample_factor = 3
    initial_samples = min(num_frames * oversample_factor, len(pairs))
    
    print(f"\nInitial sampling (oversampling by {oversample_factor}x): {initial_samples} frames")
    sampled_indices = sample_with_distribution(initial_samples, prob_dist, seed)
    
    # Enforce minimum time gap (HARD SAMPLING)
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract color and depth image pairs from MCAP ROS bags using Hard Sampling (Gaussian + minimum time gap)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
This script addresses temporal redundancy by enforcing a minimum time gap between sampled frames.
Instead of sampling many nearly-identical consecutive frames, it focuses on important events
(via Gaussian sampling) while ensuring each frame is sufficiently different (via time gap enforcement).

Examples:
  # Sample with 0.5 second minimum gap
  %(prog)s bag.mcap --config config/events.json --min-gap 0.5
  
  # Use larger minimum gap (1.0 second) for more diversity
  %(prog)s bag.mcap --config config/events.json --min-gap 1.0
  
  # Sample with command-line timestamps
  %(prog)s bag.mcap -n 100 -t 1760644700000000000 1760644750000000000 --min-gap 0.5
  
  # Override config settings
  %(prog)s bag.mcap --config config/events.json -n 200 --sigma 10.0 --min-gap 0.3
        """
    )
    parser.add_argument(
        "mcap_path",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Desired number of frames to extract (default: 100, or from config). Actual number may be less after filtering."
    )
    parser.add_argument(
        "-t", "--timestamps",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Minimum time gap between frames in seconds (default: 0.5, or from config). Higher = more diversity, less redundancy."
    )
    parser.add_argument(
        "-o", "--output-base-dir",
        type=str,
